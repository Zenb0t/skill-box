...
export class ContextGatherer {
  ...
  constructor(workingDirectory: string = process.cwd()) {
    ...
    this.config = getConfig().get<ContextConfig>('context')!;
  }

  /**
   * Gather full project context
   */
  async gather(pathOverrides?: string[]): Promise<ProjectContext> {
   ...
   */
  private async gatherFiles(paths: string[]): Promise<ContextFile[]> {
   ...
   */
  private async readContextFile(fullPath: string): Promise<ContextFile | null> {
    try {
      const stats = statSync(fullPath);
   ...
   */
  private detectFileType(path: string, _content: string): ContextFile['type'] {
   ...
   */
  private getTypeWeight(type: ContextFile['type']): number {
    if (!this.config.prioritization) {
      return 1.0;
    }
   ...
   */
  private async gatherStructure(): Promise<DirectoryNode> {
    const buildTree = (path: string, depth: number = 0): DirectoryNode | null => {
   ...
   */
  private async gatherMetadata(): Promise<ProjectMetadata> {
   ...
   */
  async getContextSummary(pathOverrides?: string[]): Promise<string> {
    const paths = pathOverrides || this.config.default_paths;
